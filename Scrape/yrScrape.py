...
def dateStringToTouple(string):
        """Converts a string of the form
        dd.mm.yyyy to a triple of ints.
        """
        return (int(string[0:2]), int(string[3:5]), int(string[6:10]))
...
def scrapeYr(url, filename):
    	...
    	with open(filename, 'a') as fileID:
    		...
    		for i, hours in enumerate(hourData):
    			...
    			hours = hours.split('-')
    			hours = (int(hours[0]),(hours[1]))
    			fileID.write(str(yrDate.yrDate(dateStringToTouple(currentDate), hours)) + ' ' + str(tempData[i]) + '\n')
    	fileID.close()
    	return None
